...
To achieve improved calibration properties, \cite{Bassetti2018-qr} extended the BLP approach in Equation~\eqref{eq:blp} by introducing a finite or infinite beta mixture. The resulting predictive cdf is
\begin{align}
\label{eq:extendblp}
...
The essence of aforementioned nonlinear pooling methods is to take ingenious transformations that may be nonlinear to either the component forecasts or the linearly pooled forecast in order to restore calibration and sharpness. While \cite{Kapetanios2015-bb} generalized the literature by accommodating the dependence of the mixture weights on the variable one is trying to forecast, allowing the weights themselves to introduce the nonlinearities. The extended BLP approach in Equation~\eqref{eq:extendblp} can be interpreted in terms of the generalized aggregation scheme developed by \cite{Kapetanios2015-bb}
...
\begin{itemize}
  ...
  \item Flexible Bayesian forecast combination structures.
\end{itemize}

\paragraph{Bayesian model averaging}

Bayesian Model Averaging (BMA) provides a conceptually elegant and logically coherent solution to the problem of accounting for model uncertainty \citep[e.g.,][]{Leamer1978-sp,Draper1995-cd,Raftery1997-ij,Garratt2003-kh}. Under this approach, the posterior probability forecast is computed by mixing a set of individual probability forecasts, $p_{i}(y_{t+h}|I_{t})=p(y_{t+h}|I_{t},M_{i})$, from model $M_{i}$, and given as
\begin{align}
\label{eq:BMA}
...
BMA in Equation~\eqref{eq:BMA} can be viewed as a linear pooling of individual probability forecasts in Equation~\eqref{eq:linear_pool}, weighted by their posterior model probabilities given in Equation~\eqref{eq:post_prob}. The approach provides a general way to deal with model uncertainty and does not require the use of conjugate families of distributions.
...
The BMA procedure is consistent in the sense that the posterior probability in Equation~\eqref{eq:post_prob} indicates the probability that model $M_{i}$ is the best under the KLIC measure distance given the sample data and reflects how well this particular model matches the observations \citep{Fernandez-Villaverde2004-pz,Raftery2005-vu,Wright2008-bs}. The BMA weights in Equation~\eqref{eq:post_prob} differ from the optimal weights in Equation~\eqref{eq:weight_klic} in assuming that the true model is included in the model space to be considered. Under this assumption, when the sample size tends to infinity, the posterior probabilities converge to zero, except for one which converges to unity. Thus, BMA is equivalent to model selection when the sample is large, with the best model (which is the true model if that exists, but is still well defined if none of the models is in fact true) receiving a weight very close to one; see \cite{Geweke2010-zn} for an empirical demonstration. In this regard, the ensemble forecast derived from BMA may be misspecified when the model space is incomplete (i.e. all models under consideration are incorrect), leading to the issue of model incompleteness. While optimal weights defined in Equation~\eqref{eq:weight_klic} do not suffer from the model incompleteness issue because the weights need not converge to zero or unity regardless of whether the component models are correct or not.

While BMA is theoretically attractive, there are two major difficulties with implementing BMA in practice. One is how to properly specify the model space under consideration to address the aforementioned issue of model incompleteness. It is often unrealistic to use the complete set of models when the number of possible models is large or their structures are complex. This difficulty can be mostly addressed by selecting a subset of models that are supported by the data or through stochastic search algorithms over the model space; see \cite{Hoeting1999-qn} and \cite{Koop2003-nl} for more details on model search strategies.

Another practical difficulty in implementing BMA is the estimation of the marginal likelihood according to Equation~\eqref{eq:marginal_likelihood}. The integrals required to obtain the marginal likelihood may be analytically intractable in many cases besides in the generalized linear regression model with conjugate priors. Therefore, approximation methods of integration such as the Laplace method or Markov Chain Monte Carlo (MCMC) methods are frequently considered to overcome the difficulty. We refer to \cite{Bassetti2020-uh} for a recent survey on the MCMC simulation algorithms used for BMA.
