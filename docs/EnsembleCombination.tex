...
\begin{align}
\tilde{P}(y_{t+h}|I_{t}) = g^{-1}\left(\sum_{i=1}^{N} w_{t+h|t,i} g\left(P_{i}(y_{t+h}|I_{t})\right)\right), \label{eq:glp}
\end{align}
...
\begin{itemize}
  \item The use of conjugate families of distributions.
  \item Bayesian model averaging.
  \item Flexible Bayesian forecast ensemble structures.
\end{itemize}

\paragraph{Bayesian model averaging}

Bayesian Model Averaging (BMA) provides a conceptually elegant and logically coherent solution to the issue of accounting for model uncertainty \citep[see, e.g.,][]{Leamer1978-sp,Draper1995-cd,Raftery1997-ij,Garratt2003-kh}. Under this approach, the posterior probability forecast is computed by mixing a set of individual probability forecasts, $p_{i}(y_{t+h}|I_{t})=p(y_{t+h}|I_{t},M_{i})$, from model $M_{i}$, and given as
\begin{align}
\label{eq:BMA}
\tilde{P}(y_{t+h}|I_{t}) = \sum_{i=1}^{N} P(M_{i}|I_{t})p(y_{t+h}|I_{t},M_{i}),
\end{align}
where $P(M_{i}|I_{t})$ is the posterior probability of model $M_{i}$. The decision makers update the prior probability of model $M_{i}$ being the true model, $P(M_{i})$, to compute the posterior probability
\begin{align}
\label{eq:post_prob}
...
\end{align}
is the marginal likelihood of model $M_{i}$, $P\left(\boldsymbol{\theta}_{i} | M_{i}\right)$ is the prior on the unknown parameters $\boldsymbol{\theta}_{i}$ conditional on model $M_{i}$, and $P\left(I_{t} | M_{i}, \boldsymbol{\theta}_{i}\right)$ is the likelihood function of model $M_{i}$.

BMA in Equation~\eqref{eq:BMA} can be viewed as a linear pooling of individual probability forecasts in Equation~\eqref{eq:linear_pool}, weighted by their posterior model probabilities given in Equation~\eqref{eq:post_prob}. The approach provides a general way to deal with model uncertainty and does not necessarily require the use of conjugate families of distributions.
%The major difficulties lie in the specifications of the model space to be considered, the model priors, $P(M_{i})$, and the parameter priors, $P\left(\boldsymbol{\theta}_{i} | M_{i}\right)$.

The BMA procedure is consistent in the sense that the posterior probability in Equation~\eqref{eq:post_prob} indicates the probability that model $M_{i}$ is the best under the KLIC measure distance and shows how well the model fits the observations \citep{Fernandez-Villaverde2004-pz,Raftery2005-vu,Wright2008-bs}. The BMA weights in Equation~\eqref{eq:post_prob} differ from the optimal weights in Equation~\eqref{eq:weight_klic} in definitely assuming that the true model is included in the model space to be considered. Under this assumption, when the sample size tends to infinity, the posterior probabilities converge to zero, except for one which converges to unity. Thus, BMA reduces to model selection for a large sample size, with the best model (which is the true model if that exists, but is still well defined if none of the models is in fact true) receiving a weight very close to one; see \cite{Geweke2010-zn} for an empirical demonstration. In this regard, the ensemble forecast derived from BMA may be misspecified when the model space is incomplete (i.e. all models under consideration are incorrect), arising the issue of model incompleteness. While optimal weights defined in Equation~\eqref{eq:weight_klic} do not suffer from these issues because the weights need not converge to zero or unity regardless of whether the component models are correct or not.

While theoretically attractive, BMA has two major challenges when implemented in practice. One is how to properly specify the model space under consideration to address the aforementioned issue of model incompleteness. It is often unrealistic to use the complete set of models when the number of possible models is large or their structures are complex. This difficulty can mostly be resolved via the selection of a subset of models that are supported by the data or through stochastic search algorithms over the model space; see \cite{Hoeting1999-qn} and \cite{Koop2003-nl} for more details on model search strategies. Another practical obstacle is the estimation of the marginal likelihood according to Equation~\eqref{eq:marginal_likelihood}. The integrals required to obtain the marginal likelihood may be analytically intractable in many cases besides, except for the generalized linear regression models with conjugate priors. Approximation methods of integration, such as the Laplace method and Markov Chain Monte Carlo (MCMC) methods, are therefore frequently used to overcome the difficulty. We refer to \cite{Bassetti2020-uh} for a recent survey on the use of MCMC procedures.


%in section 3.4.2, Bayes' Theorem is used to update the decision maker's prior distribution in the light of these data from the experts that takes the form of the joint density, or likelihood, derived from their N densities. The difficulty faced by the decision maker is deciding upon the form of the likelihood function; the likelihood must capture the bias and precision of the experts' densities as well as their dependence.
...
% refer to \cite{Winkler1981-bn} (page 2 paragraph 2-3) for dependence between distributions.
