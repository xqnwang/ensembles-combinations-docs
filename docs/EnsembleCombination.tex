...
Probability forecasts, as the most complete form of probabilistic forecasts, strive to predict the probability distribution of quantities or events of interest. We consider $N$ individual probability forecasts of a random variable $Y$ to be predicted at time $t+h$, denoted $p_{i}(y_{t+h}|I_{t})$, $i=1,\ldots,N$, using the information available up to time $t$, $I_{t}$. One popular approach is to take a linear combination of these $N$ individual probability forecasts directly, which can be viewed as a generalization of linear point forecast combinations in Equation~\eqref{eq:linear-combinations}. This approach is commonly referred to as the `linear opinion pool' in the literature on combining experts' subjective probability distributions. The linearly combined probability forecast is defined as the convex combination
\begin{align}
\label{eq:linear_opinion_pool}
p_{c}(y_{t+h}|I_{t}) = \sum_{i=1}^{N} w_{t+h,i} p_{i}(y_{t+h}|I_{t}),
\end{align}
where $w_{t+h,i}$ is the weight assigned to $i$th probability forecast. These weights are set to be non-negative and sum to one to guarantee that the combined probability forecast preserves properties of both non-negativity and integrating to one. The combined probability forecast satisfies a number of properties such as the \textit{unanimity} property (if all individual forecasters agree on a probability then the combined probability forecast agrees also); see \cite{Clemen1999-mh} for more details.

Define $\mu_{i}$ and $\sigma_{i}^{2}$ as the mean and variance of $i$th probability forecast and for simplicity drop the time subscripts. Then the linearly combined probability forecast has mean and variance
\begin{align}
...
\end{align}
These relations highlight the potential of linear combinations of probability forecasts to accommodate skewness and kurtosis (heavier tails), and also multi-modality; see \cite{Wallis2005-yf} and \cite{Hall2007-lh} for further discussion on this point. If the true (population) density is non-normal, the potential enables the combined probability forecast in Equation~\eqref{eq:variance} to mitigate misspecification of the individual probability forecasts that are normal. While combining using Equation~\eqref{eq:variance} will not be a good strategy when working with a normal population density.

Equation~\eqref{eq:variance} decomposes the variance of the combined probability forecast into the average individual variance and the disagreement on the individual means. Therefore, linear combinations of probability forecasts will in general increase the combined variance. As location (mean) diversity increases, the linear opinion pool's variance increases, leading to an underconfident and less shaper combined forecast. The result stands in contrast to that provided from combining point forecasts, where diversity benefits the combined point forecasts. The calibration problem associated with linear pooling occurs even in the ideal case in which each individual source is well-calibrated. Theoretical aspects of this finding and properties of linearly combined probability forecasts have been further studied in \cite{Hora2004-fz}, \cite{Ranjan2010-jl}, and \cite{Lichtendahl2013-rt}.
...
In principle, probability forecasts can be recalibrated before or after the combination to correct for miscalibration \cite{Turner2014-za}. However, it is challenging to estimate the degree of miscalibration, which may vary considerably among different forecasts and over time, and therefore to recalibrate accordingly. Some effort has been directed toward the development of alternative combination methods to address the calibration issue. For example, \cite{Jose2014-uh} suggested the `trimmed opinion pool', which intended to trim away some individual forecasts from a linear opinion pool before averaging. Specifically, exterior trimming that trimmed away forecasts with low or high means or cdf values served as a way to address underconfidence by decreasing the variance. Conversely, interior trimming that trimmed away forecasts with moderate means or cdf values was suggested to mitigate overconfidence via increasing the variance. The improvement offered by trimming was confirmed by \cite{Grushka-Cockayne2017-dj} at a more foundational level. Some researchers prefer to nonlinear alternatives, including generalized linear pool, the spread-adjusted linear pool, and the beta-transformed linear pool, in terms of providing better calibrated combined probability forecasts; see \cite{Gneiting2013-hl} for simulation examples and case studies. Instead of averaging probability forecasts mentioned above, \cite{Lichtendahl2013-rt} recommended to average quantile forecasts (considered in Section \ref{sec:combining_quantile_forecasts} below) based on the supportive results both theoretically and empirically.

The key practical issue is how the weights on the individual probability forecasts in linear combinations should be determined. Most simply, equal weights, are worthy of consideration. Thy are easy to understand and implement, usually yielding robust and stable outcomes. For reviews, see \cite{Wallis2005-yf} and \cite{OHagan2006-jk}. A leading example is the Survey of Professional Forecasters (SPF) in the US, which publishes the combined probability forecasts for inflation and GDP growth using equal weights. While probability forecasts provided by respondents are usually known in the form of histograms rather than continuous probability forecasts.
