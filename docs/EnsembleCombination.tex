...
The key practical issue determining the success (or failure) of a linear pooling is how the weights on the individual probability forecasts in the finite mixture should be determined. Most simply, equal weights are worthy of consideration. They are easy to understand and implement, usually yielding robust and stable outcomes. For reviews, see \cite{Wallis2005-yf} and \cite{OHagan2006-jk}. A leading example is the Survey of Professional Forecasters (SPF) in the US, which publishes the combined probability forecasts for inflation and GDP growth using equal weights. While probability forecasts provided by respondents are usually known in the form of histograms rather than continuous probability forecasts.
...
\begin{align}
\mathrm{KLIC}_{\tau+h|\tau} &= \int f(y_{\tau+h}) \ln \left\{\frac{f(y_{\tau+h})}{p_{c}(y_{\tau+h}|I_{\tau})}\right\} \mathrm{d} y_{\tau+h} \nonumber \\
&=E\left[\ln f(y_{\tau+h})-\ln p_{c}(y_{\tau+h}|I_{\tau})\right]. \nonumber
\end{align}

Under the asymptotic theory that the number of time periods $t$ grows to infinity, the minimization of the KLIC distance can be achieved by maximizing the average logarithmic score of the combined probability forecast. The use of the logarithmic scoring rule makes it convenient to determine the combination weights for probability forecasts, eliminating the need to estimate the unknown true probability. Thus, the optimal weight vector $\boldsymbol{w}_{t+h|t}$, $\boldsymbol{w}_{t+h|t}=\left(w_{t+h|t, 1}, \ldots, w_{t+h|t, N}\right)^{\prime}$, is given by
\begin{align}
\label{eq:weight_klic}
\boldsymbol{w}_{t+h|t} = \underset{\boldsymbol{w}}{\operatorname{argmax}} \frac{1}{t} \sum_{\tau=1}^{t} \ln p_{c}(y_{\tau+h}|I_{\tau}).
\end{align}

\cite{Pauwels2016-ci} examined the properties of the optimal weights in Equation~\eqref{eq:weight_klic}, centering on the asymptotic theory used by \cite{Hall2007-lh}. Both simulation and empirical results indicated that the combination with the optimal weights was inferior for small number of time periods $t$, while it minimized the KLIC distance when $t$ was sufficiently large.





Following in the footsteps of \cite{Hall2007-lh}, 

\cite{Pauwels2016-ci}, 

Many extensions and refinements of ... have been suggested.

\cite{Pauwels2016-ci} properties.


- Optimized combination weights (minimizing KLIC distance)

- Its further extensions and refinements

% - The pioneering work by Hall and Mitchell (2007) define the optimal weights as that set of weights in that minimize the KLIC distance …  (see Hall 2007; Aastveit 2018 P17). which is similar to optimal point forecast combination by minimizing MSE.
% - Optimized combination weights (KLIC; Hall 2007; Bassetti 2020 P11) and its further extensions and refinements (see Aastveit 2018 P18).
