...
\begin{align}
& \mu_{c} = \sum_{i=1}^{N} w_{i} \mu_{i}, \label{eq:mean_linear_pooling} \\
& \sigma_{c}^{2} = \sum_{i=1}^{N} w_{i} \sigma_{i}^{2} + \sum_{i=1}^{N} w_{i} \left(\mu_{i}-\mu_{c}\right)^{2}. \label{eq:variance_linear_pooling}
\end{align}
...
\boldsymbol{w}_{t+h|t} = \underset{\boldsymbol{w}}{\operatorname{argmax}} \frac{1}{t-h} \sum_{\tau=1}^{t-h} \log p_{c}(y_{\tau+h}|I_{\tau}),
\end{align}
...
%Despite its popularity, the linear opinion pool has several shortcomings, such as the lack of calibration, which have been recently discussed by Ranjan and Gneiting (2010), and Gneiting and Ranjan (2011).
%Other typical combination approaches are based on multiplicative averaging (sometimes denominated logarithmic opinion pool) or combining log odds ratios by logit regressions (Kamstra and Kennedy (1998)).
% Kamstra and Kennedy (1998) concentrate on how to combine individual probability forecasts and/or qualitative forecasts in a computationally attractive manner, through the use of logit regression.
% \cite{Clements2011-ui} (2011) expand the research initiated by Kamstra and Kennedy (1998) and study different methods for combining probability forecasts in an optimal manner and the properties of various combinations for various data generating processes. The authors argue that the optimal form of combination is unknown because the data generating process of the forecasts and the event being forecast is not known.

\subsubsection{Nonlinear pooling methods}
\label{sec:nonlinear_pooling_methods}

Despite its simplicity and popularity, the traditional linear pooling methods have several shortcomings, such as the calibration problem discussed previously. A linear pooling of probability forecasts increases the variance of the forecasts and results in a suboptimal solution, laking both calibration and sharpness. To address these shortcomings, several nonlinear alternatives to linear pooling methods have been developed with the intention of offering better calibrated and more sharper probability forecasts.

Motivated by the seminal work by \cite{Dawid1995-jj}, \cite{Gneiting2013-hl} developed a generalized linear pooling formula to incorporate a parametric family of combination formulas. Let $P_{i}(y_{t+h}|I_{t})$ denote the cdf of the probability forecast $p_{i}(y_{t+h}|I_{t})$ ($i=1,\ldots,N$), and $P_{c}(y_{t+h}|I_{t})$ denote the cdf of the combined forecast $p_{c}(y_{t+h}|I_{t})$. The generalized pooling scheme takes the following form
\begin{align}
&P_{c}(y_{t+h}|I_{t}) = h^{-1}\left(\sum_{i=1}^{N} w_{t+h|t,i} h\left(P_{i}(y_{t+h}|I_{t})\right)\right) \quad \text { or } \nonumber \\
&h\left(P_{c}(y_{t+h}|I_{t})\right) = \sum_{i=1}^{N} w_{t+h|t,i} h\left(P_{i}(y_{t+h}|I_{t})\right), \label{eq:generalized_pool}
\end{align}
where $w_{t+h|t,1},\ldots,w_{t+h|t,N}$ are nonnegative weights that sum to one, and $h$ denotes a continuous and strictly monotone function with inverse $h^{-1}$. The linear, harmonic and logarithmic (geometric) pools become special cases of Equation~\eqref{eq:generalized_pool} for $h(x)=x$, $h(x)=1/x$ and $h(x)=\log(x)$, respectively. However, \cite{Gneiting2013-hl} highlighted that the generalized pooling strategy may fail to be flexibly disperse for calibration.

The cumulative beta distribution is often employed for the purpose of recalibration because of the flexibility of its shape \citep{Graham1996-qc}. \cite{Ranjan2010-jl} proposed a Beta-transformed Linear Pool (BLP) that merged the traditional linear pool with a beta transform to aggregate individual probability forecasts while recalibrating. The BLP takes the form
\begin{align}
p_{c}(y_{t+h}|I_{t}) = B_{\alpha, \beta}\left(\sum_{i=1}^{N} w_{t+h|t,i} p_{i}(y_{t+h}|I_{t})\right),
\end{align}
where $w_{t+h|t,1},\ldots,w_{t+h|t,N}$ are nonnegative weights that sum to one, and $B_{\alpha, \beta}$ is the cdf of the beta distribution with shape parameters $\alpha > 0$ and $\beta > 0$. The BLP approach nests the traditional linear pool that arises when $\alpha = \beta = 1$. The approach can be used to aggregate probability forecasts from calibrated as well as uncalibrated sources. The parameter estimation for the BLP approach can be achieved by maximizing the average logarithmic score over the training data; see \cite{Gneiting2013-hl} for details of the optimization. A recent work by \cite{Lahiri2015-qq} proposed a simple test to evaluate probability forecasts and demonstrated the superiority of the BLP scheme over the equally weighted combination with respect to calibration and sharpness.

As an alternative to the BLP that allows one to address the calibration problem, \cite{Gneiting2013-hl} proposed the spread-adjusted pools.
