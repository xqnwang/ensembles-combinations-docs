...
# Data Import
cafe <- readRDS("/Users/wangxiaoqian/Git/quantile_ensemble_talk/cafe.rds")
auscafe <- cafe %>%
...
```{r, combination_model, eval=FALSE}
...
```{r inv_var, message=FALSE, warning=FALSE}
ft <- train %>%
  model(
    ETS = ETS(turnover),
...
```{r clear1, message=FALSE, warning=FALSE, echo=FALSE}
rm(ft)
```
...
```{r comb3, message=FALSE, warning=FALSE}
...
```{r clear2, message=FALSE, warning=FALSE, echo=FALSE}
rm(fc_s, fc_w)
```
...
```{r testcomb, message=FALSE, warning=FALSE}
# equally weighted combination
ft <- train %>%
  model(
